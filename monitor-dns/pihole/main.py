...
class PiHole(object):
    ...
    def refreshTop(self, count):
        ...
        return([self.top_queries, self.top_ads])
    ...
    def getList(self, list):
        # supported: black, regex_black, white, regex_white
        return requests.get(
    ...
    def sub(self, list, domain):
        url = "/admin/api.php?list=" + list + "&sub=" + domain + "&auth=" + self.auth_data.token
        # comment = {'comment': comment}
        response = requests.post(
            "http://" + str(self.ip_address) + url)
